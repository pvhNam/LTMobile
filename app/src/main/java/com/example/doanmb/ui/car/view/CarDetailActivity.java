...
import com.example.doanmb.core.helper.WalletHelper;
...
import com.example.doanmb.core.helper.FavoriteHelper;
...
public class CarDetailActivity extends AppCompatActivity {
    ...
    private void updateDepositInfo() {
        ...
        if (WalletHelper.requiresDeposit(days)) {
            long deposit = WalletHelper.deposit(total);
    ...
    private void sendRentRequest() {
        ...
        long pricePerDay   = parseMoney(car != null ? car.getPrice() : null);
        long total         = pricePerDay * days;
        boolean needDeposit= WalletHelper.requiresDeposit(days);
        long deposit       = needDeposit ? WalletHelper.deposit(total) : 0L;
        ...
        db.collection("orders").add(order)
                .addOnSuccessListener(ref -> {
                    ...
                    WalletHelper.holdDeposit(user.getUid(), deposit, ref.getId(),
                            new WalletHelper.Callback() {
