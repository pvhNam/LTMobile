...
import com.example.doanmb.ui.chat.view.MessagesFragment;
import com.example.doanmb.ui.profile.view.ProfileFragment;
import com.example.doanmb.ui.car.adapter.CarRentalAdapter;
import com.example.doanmb.ui.car.adapter.CarSaleAdapter;
...
public class MainActivity extends AppCompatActivity {
    ...
    private void loadFavorites() {
        ...
        if (user == null) {
            ...
            FavoriteRepository.loadIds(user.getUid(), ids -> {
                favoriteIds.clear();
    ...
    private void toggleFavorite(Car car, boolean makeFavorite) {
        ...
        if (makeFavorite) {
            favoriteIds.add(id);
            FavoriteRepository.add(user.getUid(), id);
        } else {
            favoriteIds.remove(id);
            FavoriteRepository.remove(user.getUid(), id);
        }
        if (carSaleAdapter != null) carSaleAdapter.setFavoriteIds(favoriteIds);
        if (carRentalAdapter != null) carRentalAdapter.setFavoriteIds(favoriteIds);
    }
